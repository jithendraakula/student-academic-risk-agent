...
import json
from .config import HOLDOUT_COHORT, MODEL_NAMES, MODEL_DIR
from .data_loader import load_processed_data, build_training_sets, split_by_cohort
from .predictor import predict_all_risks

def main():
    data=load_processed_data(); sets=build_training_sets(data)
    for name,spec in sets.items():
        train,test=split_by_cohort(spec["frame"],HOLDOUT_COHORT)
        assert len(train)>0 and len(test)>0
        assert set(train["student_id"]).isdisjoint(set(test["student_id"]))
