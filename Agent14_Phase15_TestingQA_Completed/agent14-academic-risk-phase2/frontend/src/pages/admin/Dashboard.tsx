...
export default function AdminDashboard() {
  ...
  const auditColumns: TableColumn<AdminAuditLog>[] = [
    { key: "action", header: "Action", render: (row) => <span className="font-semibold text-ink-900">{row.action}</span> },
    { key: "resource", header: "Resource", render: (row) => `${row.resource_type ?? "—"}${row.resource_id ? ` · ${row.resource_id}` : ""}` },
    { key: "actor", header: "Actor", render: (row) => row.actor_id ?? "System" },
