    ...
    <Card as="section" className="overflow-hidden p-0">
      <div className="relative overflow-hidden border-b border-[#cadcf2] bg-gradient-to-r from-[#eef5fd] via-white to-[#f8fbff] px-5 py-5 md:px-6">
        ...
        <div className="relative flex items-start gap-3">
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-brand-700">Academic AI support</p>
            <h2 className="mt-1 text-lg font-extrabold text-[#18345f]">{role === "hod" ? "Department Risk Analyst" : "Institutional Risk Analyst"}</h2>
            <p className="mt-1 max-w-4xl text-xs leading-5 text-slate-600">Review a concise summary of the current academic situation.</p>
          </div>
        </div>
      </div>
      <div className="space-y-4 p-5 md:p-6">
        ...
        {result ? (
          <div className="rounded-2xl border border-[#d5e2f0] bg-[#fbfdff] p-4 md:p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="text-xs font-extrabold uppercase tracking-[0.1em] text-brand-700">{result.title}</p><p className="mt-1 text-[10px] font-medium text-slate-400">{result.grounded ? "Based on current case data" : "Review with care"}</p></div>
        ...
        ) : <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-5 text-center text-xs text-slate-500">Choose an analysis above to view a concise briefing.</div>}
      </div>
    </Card>
  );
}
