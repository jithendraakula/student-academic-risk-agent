  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      <div className="overflow-x-auto">
        <table className="min-w-full md:min-w-[720px] border-collapse text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.length > 0 ? rows.map((row, index) => (
              <tr key={rowKey(row, index)} className="transition-colors duration-150 hover:bg-[#f7fbff]">
                {columns.map((column) => (
                  <td key={column.key} className={`px-4 py-3.5 align-middle text-[#364a63] ${column.className ?? ""}`}>
                    {column.render(row)}
                  </td>
                ))}
              </tr>
