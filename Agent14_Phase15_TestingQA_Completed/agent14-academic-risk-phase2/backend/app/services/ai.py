...
from app.core.config import (
    AI_ALLOWED_EXTERNAL_DATA,
    AI_INCLUDE_STUDENT_IDENTIFIERS,
)
...
from app.services.rbac import (
    can_access_student,
    can_view_support_attention_risk,
)
...
students, parents, faculty, or external systems.
                ...
                rows.append(
                    {
                        ...
                        "probability": round(
                            float(item.get("risk_probability", 0)),
                            3,
                        ),
            ...
            rows.append(
                {
                    "risk_type": risk_type,
                    "risk_score": round(
                        float(value.get("risk_score", 0)),
                        1,
                    ),
                    "probability": round(
                        float(value.get("risk_probability", 0)),
                        3,
                    ),
                    "risk_level": value.get("risk_level"),
                    "priority_score": round(
                        float(value.get("priority_score", 0)),
                        1,
                    ),
                    "confidence": value.get("confidence"),
                    "top_factors": (
                        value.get("top_factors") or []
                    )[:4],
                }
            )
...
def build_context(
    ...
    if not can_access_student(db, user, student_id):
        raise HTTPException(
            ...
            detail="Student is outside your access scope",
        )

    profile = build_student_risk_profile(
        db,
        student_id,
        user,
    )
    ...
    summaries = student_summary(
        db,
        [student_id],
        include_support_attention=can_view_support_attention_risk(
            user.role
        ),
    )
    ...
    observations = db.scalars(
        ...
        .order_by(
            AcademicObservation.observed_on.desc(),
            AcademicObservation.id.desc(),
        )
    ).all()
    ...
    if (
        AI_INCLUDE_STUDENT_IDENTIFIERS
        and not AI_ALLOWED_EXTERNAL_DATA
    ):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=(
                "External AI student-identifier sharing is not permitted "
                "by server policy"
            ),
        )

    # De-identified student context by default.
    student_context = {
    ...
    context = {
        ...
        "risk_evidence": _safe_factor_rows(
            _visible_risks(profile, user)
        ),
        "active_alerts": [
            {
        ...
        "academic_context": {
            "primary_context_intent": academic_context.get(
                "primary_context_intent"
            ),
            "primary_action_path": academic_context.get(
                "primary_action_path"
            ),
            "active_follow_up_count": academic_context.get(
                "active_follow_up_count",
                0,
            ),
            "observations": academic_context.get(
                "observations",
                [],
            )[:8],
    ...
    if intent == CopilotIntent.WHAT_IF_EXPLANATION:
        if what_if is None or not what_if.has_change():
            raise HTTPException(
                ...
                detail=(
                    "what_if values are required for "
                    "what_if_explanation"
                ),
            )

        simulation = simulate_student(
            db,
            student_id,
            user,
            what_if,
...
def _prompt_for(
    ...
    intent_instruction = {
        CopilotIntent.RISK_SUMMARY: (
            "Summarize the student's current academic risk, explain "
            "the strongest evidence, and identify the most important "
            "mentor action."
        ),
        CopilotIntent.INTERVENTION_PLAN: (
            "Create a concise, supportive intervention plan ordered "
            "by urgency and feasibility. Tie each action to a supplied "
            "risk driver."
        ),
        CopilotIntent.WHAT_IF_EXPLANATION: (
            "Explain what changes under the simulated scenario, what "
            "improved or worsened, and what the mentor should do next. "
            "Emphasize that the scenario is non-persistent."
        ),
    }[intent]
...
def _call_provider(
    ...
    if not config:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"Unsupported AI provider configuration: {AI_PROVIDER}",
        )
    ...
    try:
        ...
        if status_code in (401, 403):
            logger.warning("AI provider authentication failed: %s", detail[:200])
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="AI provider authentication failed. Please contact the administrator.",
            ) from exc

        if status_code == 429:
            logger.warning("AI provider rate limit reached: %s", detail[:200])
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail="AI assistance is temporarily rate-limited. Please try again shortly.",
            ) from exc

        if status_code in (502, 503):
            logger.warning("AI provider unavailable (%s): %s", status_code, detail[:200])
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
        ...
        if status_code == 504:
            logger.warning("AI provider timeout (%s): %s", status_code, detail[:200])
            raise HTTPException(
                status_code=status.HTTP_504_GATEWAY_TIMEOUT,
                detail="The AI response timed out. Please try again.",
            ) from exc

        logger.warning("AI provider HTTP failure (%s): %s", status_code, detail[:200])
        raise HTTPException(
        ...
        logger.warning("AI provider connection failure: %s", exc)
        raise HTTPException(
            status_code=status.HTTP_504_GATEWAY_TIMEOUT,
    ...
    try:
        ...
        logger.warning("AI provider returned malformed response: %s", raw[:500])
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="The AI returned an unexpected response.",
        ) from exc
...
def _parse_json_text(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="AI provider returned non-JSON copilot output",
        ) from exc

    if not isinstance(result, dict):
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="AI provider returned an invalid copilot payload",
        )
    ...
    return {
        ...
        "priority_rationale": str(
            result.get("priority_rationale", "")
        ),
        "cautions": [
            str(x)
            for x in (
                result.get("cautions") or []
            )
        ][:6],
    }
...
def run_copilot(
    ...
    return {
        ...
        **parsed,
        "source_snapshot": {
            "risk_source": (
                "RiskPrediction + canonical priority engine"
            ),
            "student_identifiers_sent_to_provider": (
                AI_INCLUDE_STUDENT_IDENTIFIERS
            ),
